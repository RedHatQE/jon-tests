...
public class Operations extends ResourceTab {
    ...
    public Map<String, String> assertOperationResult(Operation op, boolean success) {
        ...
        while ((tasks.cell(NOT_YET_STARTED_MESSAGE).in(tasks.div(opName).parentNode("tr")).exists()
                || tasks.div(NOT_YET_STARTED_MESSAGE).in(tasks.div(opName).parentNode("tr")).exists())
